...
void DevXhci::Init() {
  ...
  uint8_t *controlRegistersBaseAddr = nullptr;
  uint8_t controlRegistersSize = 0;
  {
    uint32_t addr_bkup;
    ...
    controlRegistersBaseAddr = reinterpret_cast<uint8_t *>(
        mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));
    controlRegistersSize = size;
    close(fd);
  }
  printf("BAR0: %p %d\n", controlRegistersBaseAddr, controlRegistersSize);
  return;

  /*
    _capreg_base_addr32 = reinterpret_cast<volatile uint32_t
    *>(_capreg_base_addr); _oopreg_base_addr = reinterpret_cast<volatile
    uint32_t *>(_capreg_base_addr + _capreg_base_addr[0]);
    */
  // 4.2 Host Controller Initialization

  // wait until ready
  while (IsFlagSet(_opreg_base_addr[kOpRegOffsetUsbSts],
                   kOpRegUsbStsFlagControllerNotReady)) {
    asm volatile("" ::: "memory");
  }

  if (IsFlagClear(_opreg_base_addr[kOpRegOffsetUsbSts],
                  kOpRegUsbStsFlagHchalted)) {
    // halt controller
    _opreg_base_addr[kOpRegOffsetUsbCmd] &= ~kOpRegUsbCmdFlagRunStop;
  }

  while (IsFlagClear(_opreg_base_addr[kOpRegOffsetUsbSts],
                     kOpRegUsbStsFlagHchalted)) {
    asm volatile("" ::: "memory");
  }
  ...
  while (
      IsFlagSet(_opreg_base_addr[kOpRegOffsetUsbCmd], kOpRegUsbCmdFlagReset)) {
    asm volatile("" ::: "memory");
  }

  // get information
  _max_slots = MaskValue<CapReg32HcsParams1MaxSlots>(
      _capreg_base_addr32[kCapReg32OffsetHcsParams1]);
  _excapreg_base_addr =
      _capreg_base_addr32 + MaskValue<CapReg32HccParams1Xecp>(
                                _capreg_base_addr32[kCapReg32OffsetHccParams1]);
  _doorbell_array_base_addr =
      _capreg_base_addr32 + MaskValue<CapReg32DboffDoorbellArrayOffset>(
                                _capreg_base_addr32[kCapReg32OffsetDboff]);
  _runtime_base_addr =
      _capreg_base_addr32 + MaskValue<CapReg32TrsoffRuntimeSpaceOffset>(
                                _capreg_base_addr32[kCapReg32OffsetRtsoff]) *
                                8;

  // Program the Max Device Slots Enabled (MaxSlotsEn) field in the CONFIG
  // register (5.4.7) to enable the device slots that system software is going
  // to use.
  _opreg_base_addr[kOpRegOffsetConfig] &=
      ~GenerateMask<OpRegConfigMaxSlotsEn, uint32_t>();
  _opreg_base_addr[kOpRegOffsetConfig] |=
      GenerateValue<OpRegConfigMaxSlotsEn, uint32_t>(_max_slots);

  // Program the Device Context Base Address Array Pointer (DCBAAP) register
  // (5.4.6) with a 64-bit address pointing to where the Device Context Base
  // Address Array is located.
  ...
  _opreg_base_addr[kOpRegOffsetCrcr] = (_command_ring.GetMemory().GetPhysPtr() &
                                        kOpRegCrcrMaskCommandRingPointer) |
                                       kOpRegCrcrFlagRingCycleStatus;
  _opreg_base_addr[kOpRegOffsetCrcr + 1] =
      _command_ring.GetMemory().GetPhysPtr() >> 32;
  ...
  _interrupter.Init(_runtime_base_addr + kRunRegIntRegSet,
                    &_event_ring_segment_table, &_event_ring);

  // start controller
  _opreg_base_addr[kOpRegOffsetUsbCmd] |= kOpRegUsbCmdFlagRunStop;

  while ((_opreg_base_addr[kOpRegOffsetUsbSts] & kOpRegUsbStsFlagHchalted) !=
         0) {
    asm volatile("" ::: "memory");
  }
  ...
  int max_ports = MaskValue<CapReg32HcsParams1MaxPorts>(
      _capreg_base_addr32[kCapReg32OffsetHcsParams1]);
...
void DevXhci::AttachAllSub() {
  ...
  for (int i = 0; i < max_ports; i++) {
    ...
    volatile uint32_t *portsc =
        &_opreg_base_addr[kOpRegOffsetPortsc + (root_port_id - 1) * 4];
...
DevUsb *DevXhci::Device::Init() {
  ...
  do {
    CommandRing::AddressDeviceCommandTrb com(_input_context.GetPhysAddr(),
                                             _slot_id, true);
    CommandRing::CompletionInfo info = _hc->_command_ring.Issue(com, &_hc->_mp);
    if (info.completion_code != TrbCompletionCode::kSuccess) {
      return nullptr;
    ...
    case UsbCtrl::PortSpeed::kFullSpeed: {
      do {
        ...
        request.MakePacketOfGetDescriptorRequest(
            UsbCtrl::DescriptorType::kDevice, 0, 8);
        if (!SendControlTransfer(request, mem, 8)) {
          return nullptr;
        }

        _input_context.UpdateMaxPacketSizeOfEndpoint0(
            mem.GetVirtPtr<UsbCtrl::DeviceDescriptor>()->max_packet_size);
      } while (0);

      do {
        CommandRing::EvaluateContextCommandTrb com(_input_context.GetPhysAddr(),
                                                   _slot_id);
        CommandRing::CompletionInfo info =
            _hc->_command_ring.Issue(com, &_hc->_mp);
        if (info.completion_code != TrbCompletionCode::kSuccess) {
          return nullptr;
        }
      } while (0);
...
bool DevXhci::Device::SendControlTransfer(UsbCtrl::DeviceRequest &request,
                                          Memory &mem, size_t data_size) {
  if (request._length == 0) {
    TransferRing::TransferTrb *trb[2];
    TransferRing::SetupStageTrb trb1(
        TransferRing::SetupStageTrb::ValueTransferType::kNoDataStage, false,
        true, request);
    TransferRing::StatusStageTrb trb2(TrbRingBase::Trb::Direction::kIn, false,
                                      true, false);
    ...
    TransferRing::CompletionInfo info =
        _input_context.Issue(1, trb, 2, &_hc->_mp);
    if (info.completion_code != TrbCompletionCode::kSuccess) {
      return false;
    }
  } else {
    ...
    TransferRing::DataStageTrb trb2(dir1, data_size, false, false, false,
                                    mem.GetPhysPtr());
    ...
    TransferRing::CompletionInfo info =
        _input_context.Issue(1, trb, 3, &_hc->_mp);
    if (info.completion_code != TrbCompletionCode::kSuccess) {
      return false;
    }
  }
  ...
  while (true) {
    ...
    if (trb.GetCycleBit() != _consumer_cycle_bit) {
      if (dequeu_ptr_incremented) {
        dequeue_ptr = _mem->GetPhysPtr() +
                      (ptr - _mem->GetVirtPtr<uint32_t>()) * sizeof(uint32_t);
      }
    ...
    switch (trb.GetType()) {
      ...
      case PortStatusChangeEventTrb::kValueTrbType: {
        ...
        pthread_t tid;
        if (pthread_create(&tid, NULL, HandlePortStatusChange, container) !=
            0) {
    ...
    ptr += 4;
    if (ptr - _mem->GetVirtPtr<uint32_t>() ==
        kEntryNum * kEntrySize / sizeof(uint32_t)) {
...
void DevXhci::Interrupter::Init(volatile uint32_t *base_addr,
                                EventRingSegmentTable *erst,
  ...
  _base_addr[kRegOffsetErstsz] =
      GenerateValue<ErstszRegEventRingSegmentTableSize, uint32_t>(
          erst->GetSize());
  _dequeue_ptr = event_ring->GetMemory().GetPhysPtr();
  WriteDequeuePtr();
  _base_addr[kRegOffsetErstba + 0] =
      erst_addr &
      GenerateMask<ErstbaEventRingSegmentTableBaseAddress, uint32_t>();
...
uint8_t DevXhci::GetSlotType(int root_port_id) {
  ...
  while (true) {
    uint8_t capid = MaskValue(*base, kExtCapRegLenCapabilityId,
                              kExtCapRegOffsetCapabilityId);
    switch (capid) {
      case kExCapCodeSupportedProtocol: {
        int offset =
            MaskValue(base[2], kSupportedProtocolCap8LenCompatiblePortOffset,
                      kSupportedProtocolCap8OffsetCompatiblePortOffset);
        int count =
            MaskValue(base[2], kSupportedProtocolCap8LenCompatiblePortCount,
                      kSupportedProtocolCap8OffsetCompatiblePortCount);
        if (offset <= root_port_id && root_port_id < offset + count) {
          return MaskValue(base[3], kSupportedProtocolCapCLenProtocolSlotType,
                           kSupportedProtocolCapCOffsetProtocolSlotType);
        }
        break;
      }
    }
    int next_offset =
        MaskValue(*base, kExtCapRegLenNextPointer, kExtCapRegOffsetNextPointer);
    if (next_offset == 0) {
      break;
    }
    base += next_offset;
...
void DevXhci::SetupScratchPad() {
  // setup scratchpad
  int max_scratchpad_bufs = (MaskValue<CapReg32HcsParams2MaxScratchpadHi>(
                                 _capreg_base_addr32[kCapReg32OffsetHcsParams2])
                             << 5) |
                            MaskValue<CapReg32HcsParams2MaxScratchpadLow>(
                                _capreg_base_addr32[kCapReg32OffsetHcsParams2]);
  _context_size = IsFlagSet(_capreg_base_addr32[kCapReg32OffsetHccParams1],
                            kCapReg32HccParams1FlagContextSize)
                      ? 64
                      : 32;

  if (max_scratchpad_bufs > 0) {
    size_t page_size =
        (_opreg_base_addr[kOpRegOffsetPageSize] & kOpRegPageSizeMask) << 12;
    if (page_size != 4096) {
...
DevUsb *DevXhci::AttachDevice(Hub *hub, int hub_addr, int hub_port_id) {
  HubPortDevice *device =
      new HubPortDevice(this, _device_list[hub_addr], hub, hub_port_id);
  return device->Init();
}

void DevXhci::Device::DeviceContext::OutEndpointContext::Init(
    Device *device, uint32_t *addr, int dci, int interval,
    UsbCtrl::TransferType type, int max_packet_size) {
...
void DevXhci::Device::DeviceContext::InEndpointContext::Init(
    Device *device, uint32_t *addr, int dci, int interval,
    UsbCtrl::TransferType type, int max_packet_size,
...
int DevXhci::Device::InputDeviceContext::Init(Device *device) {
  ...
  _control_context.Init(addr +
                        (0 * _device->_hc->_context_size) / sizeof(uint32_t));
  _dev_context._slot_context.InitInput(
      _device, addr + (1 * _device->_hc->_context_size) / sizeof(uint32_t));
  ...
  _dev_context._in_endpoint_context[0].Init(
      _device, addr + (2 * _device->_hc->_context_size) / sizeof(uint32_t), 1,
      0, UsbCtrl::TransferType::kControl, _ed0_max_packet_size, nullptr);

  return 0;
}

void DevXhci::BufferingNormalTrbHandler::Handle() { _ring->Handle(_index); }
